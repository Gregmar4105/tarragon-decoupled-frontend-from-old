            ...
            <main className="flex-1">
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-12 -mt-10 mb-24 relative z-10">
                    <div className="grid md:grid-cols-3 gap-8">
                        {plans.map((plan: any) => (
                            <div key={plan.id} className={`rounded-3xl shadow-xl border p-8 flex flex-col relative overflow-hidden transform transition-transform duration-300 ${plan.is_popular ? 'bg-orange-600 shadow-orange-200 border-orange-500 scale-105 z-10' : 'bg-white border-gray-100 hover:-translate-y-1'}`}>
                                {plan.is_popular && (
                                    <div className="absolute top-0 inset-x-0 bg-orange-500/50 py-1 text-center text-xs font-bold text-white uppercase tracking-wider">
                                        Most Popular
                                    </div>
                                )}
                                ...
                                <p className={`mb-6 ${plan.is_popular ? 'text-orange-100' : 'text-gray-500'}`}>{plan.subtitle}</p>
                                <div className="flex items-baseline gap-1 mb-6">
                                    <span className={`text-sm font-medium mr-1 ${plan.is_popular ? 'text-orange-100' : 'text-gray-500'}`}>From</span>
                                    <span className={`text-4xl font-extrabold ${plan.is_popular ? 'text-white' : 'text-gray-900'}`}>{format(plan.price_small)}</span>
                                    <span className={`font-medium ${plan.is_popular ? 'text-orange-200' : 'text-gray-500'}`}>/{plan.billing_cycle}/bag</span>
                                </div>
                                {plan.description && (
                                    <p className={`text-xs -mt-4 mb-6 ${plan.is_popular ? 'text-orange-200' : 'text-gray-400'}`}>{plan.description}</p>
                                )}

                                <ul className={`space-y-4 mb-8 flex-1 ${plan.is_popular ? 'text-orange-50' : ''}`}>
                                    {plan.features?.map((feature: string, idx: number) => (
                                        <li key={idx} className="flex items-center gap-3 text-sm">
                                            {plan.is_popular ? (
                                                <div className="bg-orange-500/50 p-1 rounded-full"><Check className="w-3 h-3 text-white" /></div>
                                            ) : (
                                ...
                                <BookingModal>
                                    <Button className={`w-full font-bold h-12 rounded-xl ${plan.is_popular ? 'bg-white text-orange-600 hover:bg-orange-50 shadow-lg' : 'bg-orange-50 text-orange-700 hover:bg-orange-100'}`}>
