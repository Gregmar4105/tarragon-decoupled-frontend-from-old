...
class EmailSettingsController extends Controller
{
    ...
    public function testConnection(Request $request)
    {
        ...
        try {
            ...
            // Restore config (optional but good practice in a long running process, 
            // though here it's a fresh request)
            Config::set('mail', $backup);

            return response()->json(['success' => true, 'message' => 'Successfully sent a test email to ' . $user->email]);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Failed to send test email: ' . $e->getMessage()], 400);
        }
    }
}
